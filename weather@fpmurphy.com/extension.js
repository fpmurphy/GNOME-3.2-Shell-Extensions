...
PanelItemTooltip.prototype = {
    _init: function(panelItem, initTitle) {
        this._tooltip = new St.Tooltip({styleClass: 'weather-tooltips' });
        this._tooltip.set_label(initTitle);
    ...
    show: function() {
        if (this.blockShow || this.preventShow || global.menuStackLength > 0) return;
        ...
        let tooltipTop = this._panelItem.actor.get_allocation_box().y2;
        let midpoint = Math.round((this._panelItem.actor.get_allocation_box().x2 - this._panelItem.actor.get_allocation_box().x1)/2);
        let [x, y] = this._panelItem.actor.get_transformed_position();

        let tooltipLeft = x + midpoint - Math.round(tooltipWidth/2);
...
function PrefsDialog(path) {
    this._init(path);
}

PrefsDialog.prototype = {
    ...
    _unitswitch_button: function(actor, event) {
        ...
        if (event.get_button() == 1 || event.get_button() == 3) {
            this._unitswitch_set(!this._switch.state);
        }
    },
...
function WeatherPanelButton() {
    this._init.apply(this, arguments);
}

WeatherPanelButton.prototype = {
    ...
    _init: function(path) {
        ...
        this._weatherIconBox.child =  new St.Icon({icon_name: 'view-refresh-symbolic', 
                                          icon_size: 22, 
        ...
        this._tooltip = new PanelItemTooltip(this, "");       

        this.mainBox = new St.BoxLayout({ vertical: true, style_class: 'weather-box' });
        this.mainBox.hide();
        this.menu.addActor(this.mainBox);
        this._getWeatherInfo(false);

        this.actor.add_actor(this._weatherIconBox);
        this.menu.connect('open-state-changed', Lang.bind(this, function(menu, isOpen) {
            if (isOpen) { 
                this._tooltip.block(true);
                this._displayWeatherPanel();
            } else 
                this._tooltip.block(false);
        }));
    ...
    _getWeatherInfo: function(mode) {
        ...
        this._prefsDialog.get_apikey(), function(weatherinfo) {
            ...
            let icon_uri = file.get_uri();
            let icon = St.TextureCache.get_default().load_uri_sync(1, icon_uri, 22, 22);
            this._weatherIconBox.child = icon; 
            this._tooltip.set_text((this._prefsDialog.get_units() > 0 ? curr[0].temp_C + 'C' : curr[0].temp_F + 'F') );
        });
    ...
    _displayWeatherPanel: function() {
        ...
        this.mainBox.get_children().forEach(function (actor) { actor.destroy(); });
        ...
        this.mainBox.add_actor(this._locationWeather, { expand: false, x_fill: false });
        this.mainBox.add(_horzSep1);
        this.mainBox.add_actor(_observationBox, { expand: false, x_fill: false });
        this.mainBox.add_actor(this._currentWeather, { expand: false, x_fill: false });
        this.mainBox.add(_horzSep2);
        this.mainBox.add_actor(this._futureWeather, { expand: false, x_fill: false });
        ...
        _observationBox.add_actor(new St.Label({ style_class: 'weather-observation',
                                  text: _('Observation time at location: ') + weathers[0].date + " " + curr[0].observation_time }));
        }
        ...
        lbCaptions.add_actor(new St.Label({ text: _('Visibility:'), style_class: 'weather-captions-top'}));
        lbValues.add_actor(currVisibility);
        lbCaptions.add_actor(new St.Label({ text: _('Humidity:'), style_class: 'weather-captions-bottom'}));
        ...
        mbCaptions.add_actor(new St.Label({ text: _('Pressure:'), style_class: 'weather-captions-top'}));
        mbValues.add_actor(currPressure);
        mbCaptions.add_actor(new St.Label({ text: _('Wind:'), style_class: 'weather-captions-bottom'}));
        ...
        rbCaptions.add_actor(new St.Label({ text: _('Cloud Cover:'), style_class: 'weather-captions-top'}));
        rbValues.add_actor(currCloudCover);
        rbCaptions.add_actor(new St.Label({ text: _('Precipitation:'), style_class: 'weather-captions-bottom'}));
...
function init(metadata) {
    return new WeatherPanelButton(metadata.path);
}
