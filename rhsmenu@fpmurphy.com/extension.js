...
ApplicationsButton.prototype = {
    ...
    _init: function(icon_path, menu_orientation) {
        ...
        // icon_type can be FULLCOLOR or SYMBOLIC
    ...
    _display : function() {
        ...
        let searchBin = new St.Bin({ x_align: St.Align.END });
        searchBin.add_actor(this.searchBox);
        leftPane.add_actor(searchBin);
        ...
        leftPane.add_actor(this.categoriesApplicationsBox);
    ...
    _displayButtons: function(apps, places) {
         if (apps) {
            for (var i=0; i<apps.length; i++) {
               let app = apps[i];            
               let applicationButton = new ApplicationButton(app, this.menu);        
         ...
         if (places) {
            for (var i=0; i<places.length; i++) {
               let place = places[i];            
               let button = new PlaceButton(place, place.name, this.menu);               
...
GreatMenu.prototype = {
    ...
    enable: function() {  
        ...
        Main.panel._rightBox.insert_actor(this._appsMenuButton.actor, children.length - 1 );    
    ...
    disable: function() {
        Main.panel._rightBox.remove_actor(this._appsMenuButton.actor);    
